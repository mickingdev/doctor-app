...
const Blog = () => {
    ...
    const Content = () =>{
        ...
        if (!isLoading && isError) return <div>{message.error('something went Wrong!')}</div>
        if (!isLoading && !isError && blogData?.length === 0) return <Empty />
        if (!isLoading && !isError && blogData?.length > 0) return
            <>
                {
                    blogData?.map((item) => (
